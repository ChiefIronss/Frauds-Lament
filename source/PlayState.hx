...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(curStage)
		{
			...
			case 'space':
				var evilTrail = new FlxTrail(dad, null, 1, 6, 0.7, 0.069); //ah hell nah
				insert(members.indexOf(dadGroup) - 1, evilTrail);
		}
		...
		if (isStoryMode && !seenCutscene)
		{
			...
			{
				case "monster" | "mauso":
					var whiteScreen:FlxSprite = new FlxSprite(-500, -500).makeGraphic(Std.int(FlxG.width * 10), Std.int(FlxG.height * 10), FlxColor.WHITE);
					add(whiteScreen);
					whiteScreen.scrollFactor.set();
					whiteScreen.blend = ADD;
					camHUD.visible = false;
					snapCamFollowToPos(boyfriend.getMidpoint().x + 40, boyfriend.getMidpoint().y - 50);
					inCutscene = true;
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['week1_nomiss', 'week2_nomiss', 'week3_nomiss', 'week4_nomiss',
				...
				'deso_complete', 'impsyn_complete', 'mauso_complete', 'dioko_complete',
				'astra_complete',
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'deso_complete':
						if(Paths.formatToSongPath(SONG.song) == 'desolation' && !usedPractice) {
							unlock = true;
						}
					case 'impsyn_complete':
						if(Paths.formatToSongPath(SONG.song) == 'impostor-syndrome' && !usedPractice) {
							unlock = true;
						}
					case 'mauso_complete':
						if(Paths.formatToSongPath(SONG.song) == 'mauso' && !usedPractice) {
							unlock = true;
						}
					case 'dioko_complete':
						if(Paths.formatToSongPath(SONG.song) == 'diokophobia' && !usedPractice) {
							unlock = true;
						}
					case 'astra_complete':
						if(Paths.formatToSongPath(SONG.song) == 'astraphobia' && !usedPractice) {
							unlock = true;
						}
					case 'ur_bad':
